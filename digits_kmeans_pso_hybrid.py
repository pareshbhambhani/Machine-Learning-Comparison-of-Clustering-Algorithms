...
def kmeans(data):
    ...
    return Fdigits(cluster1,cluster2,cluster3,cluster4,cluster5,cluster6,cluster7,cluster8,cluster9,cluster10,centroids), labels, centroids
...
def silhouette_samples(X, labels, metric='euclidean'):
    ...
    print "Inter cluster distance is:", B
    print "Intra cluster distance is:", A    
...
class Particle_digits:
    ...
    def init_position (self):
        ...
        for i in range (Nc_digits):
            for j in range (len(data_digits[0])):
                temp[i][j] = np.random.normal(loc=0.0, scale=1.0)
        return temp        
    ...
    def update_vel (self,g_best_pos):         
        ...
        for t in range (len(self.dvelocity)):
            for j in range (len(data_digits[0])):
                new_velocity[t][j] = dampener*w*self.dvelocity[t][j] + c1 * r1 * (self.dbest_pos[t][j] - self.dpos[t][j]) + c2 * r2 * (g_best_pos[t][j] - self.dpos[t][j])
...
#            print "Best Position: ", best_pos_digits
    ...
    while (km):
        try:
            ...
            """K-Means"""
            Kmeans_error,labels_kmeans, centroids_kmeans = kmeans(data_digits)
...
#            print "Best Position PSO hybrid: ", best_pos_digits
            print "---------PSO-Hybrid error is--------- ", best_err_digits    
            label_pso_array = np.asarray(label_pso(data_digits,best_pos_digits))
            print "---------PSO-Hybrid silhouette score is-------",silhouette_score(data_digits,label_pso_array)
...
#                    print "Some clusters in digits dataset bad. Try again"
            ...
            # Initialize the best position, velocity, error for  digits datasets
            best_pos_digits = []
            best_velocity_digits = []
            best_err_digits = -1
            i = 0  
...
##########################################################################################
            ## Visualize the PSO-Hybrid results for PCA-reduced data
            #    
            ## Initialize the swarm
...
#            print "---------PSO-Hybrid error is--------- ", best_err_digits    
            ...
            # Plot the centroids as a white X
            plt.scatter(best_pos_digits[:, 0], best_pos_digits[:, 1],marker='x', s=169, linewidths=3,color='w', zorder=10)
            plt.title('PSO-Hybrid clustering on the digits dataset (PCA-reduced data)\n''Centroids are marked with white cross')
            plt.xlim(x_min, x_max)
            plt.ylim(y_min, y_max)
            plt.xticks(())
            plt.yticks(())
